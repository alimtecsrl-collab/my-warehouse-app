...
def get_data(sheet_name):
    """Исправленная функция: без дублей и с принудительным обновлением кэша"""
    try:
        ...
        for col in SCHEMAS[sheet_name]:
            if col not in df.columns:
                # Заполняем нулями финансовые поля, иначе пустой строкой
...
if choice == "📊 Склад":
    ...
    if not df.empty:
        ...
        if not to_print.empty:
            if st.button(f"📄 Сформировать лист QR для {len(to_print)} поз.", type="primary"):
                ...
                st.download_button("📥 Скачать файл", data=sheet, file_name=f"qr_labels_{date.today()}.png")
    ...
    with st.form("in_form", clear_on_submit=True):
        ...
        p_price = col2.number_input("Цена закупки (ед. Lei)", min_value=0.0, value=0.0)
        m_stock = col2.number_input("Мин. остаток", min_value=0.0, value=5.0)
        ...
        if st.form_submit_button("✅ Принять"):
            ...
            if final_name and b_num:
                ...
                new_trans = pd.DataFrame([{"id": t_id, "batch_id": new_id, "type": "IN", "quantity": round(float(qty), 2),
    ...
    c_new = col_c2.text_input("Имя клиента", disabled=(c_sel != "+ Новый клиент..."))
    ...
    if not df_avail.empty:
        scanned_id = None
        tab_cam, tab_list = st.tabs(["📷 Сканировать QR", "🔎 Список"])
        with tab_cam:
            cam_photo = st.camera_input("QR-код")
            if cam_photo:
                qr_val = read_qr_from_image(cam_photo)
                if qr_val:
                    try: scanned_id = int(float(str(qr_val).strip()))
                    except: st.error("Ошибка кода")
        with tab_list:
            items = df_avail.apply(lambda x: f"ID:{x['id']} | {x['Товар']} ({x['Партия']}) | Ост: {x['Остаток']:.2f}", axis=1).tolist()
            manual_choice = st.selectbox("Поиск вручную", ["Не выбрано"] + items)
            if manual_choice != "Не выбрано": scanned_id = int(float(manual_choice.split("|")[0].replace("ID:", "")))
        if scanned_id:
            ...
            if not match.empty:
                ...
                with col_q1:
                    qty_to_add = st.number_input("Количество", min_value=0.01, max_value=max(0.01, available_now), value=min(1.0, available_now))
                    sell_price = st.number_input("Цена продажи (Lei)", min_value=0.0, value=0.0)
                    if st.button("➕ В корзину"):
                        st.session_state.cart.append({"batch_id": scanned_id, "item_name": row['Товар'], "qty": round(float(qty_to_add), 2), "price": sell_price})
                        st.rerun()
        ...
        if st.session_state.cart:
            ...
            st.dataframe(display_cart[['item_name', 'qty', 'price']].rename(columns={"item_name":"Товар", "qty":"Кол-во", "price":"Цена"}), use_container_width=True)
            col_b1, col_b2 = st.columns(2)
            with col_b1:
                if st.button("🔥 Провести списание", type="primary"):
                    if not final_client: st.error("Укажите клиента!")
                    else:
                        if c_sel == "+ Новый клиент...": safe_update("clients", pd.concat([df_clients, pd.DataFrame([{"name": final_client}])], ignore_index=True))
                        df_t = get_data("transactions")
                        t_id_start = 1 if df_t.empty else int(pd.to_numeric(df_t['id']).max()) + 1
                        new_transactions = []
                        for i, row in display_cart.iterrows():
                            new_transactions.append({"id": t_id_start + i, "batch_id": row['batch_id'], "type": "OUT", "quantity": row['qty'], "price": row['price'], "buyer": final_client, "date": str(date.today()), "month": date.today().month, "year": date.today().year})
                        if safe_update("transactions", pd.concat([df_t, pd.DataFrame(new_transactions)], ignore_index=True)):
                            st.success("Готово!"); st.session_state.cart = []; time.sleep(1); st.rerun()
            with col_b2:
                if st.button("🗑 Очистить"): st.session_state.cart = []; st.rerun()
